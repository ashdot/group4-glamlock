...
def register():
    ...
    elif form.errors: #Handles form errors
        print("\n!!! FORM ERRORS !!!")
        print(form.errors)
...
def login():
    ...
    if form.validate_on_submit():
        try:
            ...
            if not user: #If user is not present it redirects to login
            ...
            if user and user.check_password(form.password.data): #If user is present and password is right system logs in user 
